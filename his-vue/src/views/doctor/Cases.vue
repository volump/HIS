<template>
  <div>
    <Breadcrumb msg1="医生工作站" msg2="挂号列表" />
    <el-tag type="warning">已挂号:{{state_1}}</el-tag>
    <el-tag type="warning">已接诊:{{state_2}}</el-tag>
    <el-tag type="warning">已退号:{{state_3}}</el-tag>
    <!-- 卡片视图区域 -->
    <el-card>
      <el-row :gutter="10">
        <el-col :span="10">
        ...
        response => {
          ...
          for(var i=0;i<response.data.records.length;i++){
              if(response.data.records[i].status=='1')
                  this.state_1++;
              if(response.data.records[i].status=='2')
                  this.state_2++;
              if(response.data.records[i].status=='3')
                  this.state_3++;
          }
