<template>
  <div>
      ...
      >
        ...
        <el-table-column label="状态">
          <template slot-scope="scope">
            <el-tag>{{inspect_status[scope.row.status]}}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="'添加'+title+'项目'" :visible.sync="show" :close-on-click-modal="false" :show-close="false" width="1200px">
      <Add v-if="show" :title="title" :show.sync="show" :caseId="caseId" url="inspect"></Add>
...
export default {
  ...
  methods: {
    getData () {
      this.$axios.get(`/inspectApplys/list/${this.caseId}`,
        response => {
    ...
    del (id) {
      this.$axios.del(`/inspectApplys/${id}`, () => { this.getData() })
    },
    check (item) {
      this.$axios.put(`/inspectApplys/check/${item.id}`, () => { item.status = 3 })
    }
  }
}
</script>
